...
// Relay configuration for streaming worker
std::string g_relay_host;
int g_relay_port = 8554;
...
bool spawnStreamingWorker()
{
    ...
    args << "\"" << selfPath << "\""
         << " --streaming-worker"
         << " --relay-host=" << g_relay_host
         << " --relay-port=" << g_relay_port;
    ...
    g_rdp_worker_pi = pi;
    log("[worker] Streaming worker process started, pid=" + std::to_string(pi.dwProcessId));
    return true;
}
                ...
                if (r == 1)
                {
                    ...
                    if (type == "command")
                    {
                        ...
                        if (RDPAgent::json_str(msg, "cmd", cmd))
                        {
                            ...
                            {
                                RDPAgent::json_str(msg, "relay_host", g_relay_host);
                                RDPAgent::json_int(msg, "relay_port", g_relay_port);
...
// Thread: monitor streaming worker process independently of WebSocket connection
static void inactivity_monitor_thread()
{
...
void mainLogic()
{
    ...
    // === INIT CONFIG (без запуска worker — ждём команду от сервера) ===
    {
    ...
    // Поток для приёма команд управления (start-streaming / stop-streaming)
    std::thread cmdThread(controlCommandLoop);

    // Поток мониторинга состояния worker (работает независимо от WebSocket)
...
// Forward declaration: streaming worker defined after main()
int run_streaming_worker(const std::string &relay_host, int relay_port);

int main(int argc, char *argv[])
{
    // ---- parse args ----
    bool worker_mode = false;
    std::string cli_relay_host;
    int cli_relay_port = 8554;
    for (int i = 1; i < argc; ++i)
    {
        std::string a = argv[i];
        if (a == "--streaming-worker")
            worker_mode = true;
        else if (a.rfind("--relay-host=", 0) == 0)
            cli_relay_host = a.substr(13);
        else if (a.rfind("--relay-port=", 0) == 0)
        {
            try { cli_relay_port = std::stoi(a.substr(13)); }
            catch (...) {}
        }
    }
    ...
    if (worker_mode)
    {
        setupFileLogger("agent_streaming.log");
        log("=== Starting in STREAMING-WORKER mode ===");
        return run_streaming_worker(cli_relay_host, cli_relay_port);
    }
...
// Minimal RTSP push client for sending H.264 over interleaved TCP
class RtspPushClient
{
public:
    RtspPushClient() {}
    ~RtspPushClient() { Disconnect(); }
    ...
    bool Announce(const std::string &stream_name, int width, int height,
                  const std::vector<uint8_t> &sps, const std::vector<uint8_t> &pps)
    {
        // Base64 encode SPS and PPS for SDP
        std::string sps_b64 = base64_encode(sps.data(), sps.size());
        std::string pps_b64 = base64_encode(pps.data(), pps.size());

        m_base_url = "rtsp://" + m_host + ":" + std::to_string(m_port) + "/" + stream_name;
        ...
        // OPTIONS
        std::string resp;
        if (!SendRtspCommand("OPTIONS", m_base_url, "", resp))
            return false;

        // ANNOUNCE
        if (!SendRtspCommand("ANNOUNCE", m_base_url, sdp, resp))
            return false;
        ...
        if (sess_pos == std::string::npos)
            return false;
        sess_pos += 9;
        size_t sess_end = resp.find("\r\n", sess_pos);
        if (sess_end == std::string::npos)
            sess_end = resp.find(";", sess_pos);
        m_session = resp.substr(sess_pos, sess_end - sess_pos);

        // SETUP
        std::string track_url = m_base_url + "/track1";
        std::string setup_cmd = "Transport: RTP/AVP/TCP;unicast;interleaved=0-1\r\n";
        if (!SendRtspCommand("SETUP", track_url, setup_cmd, resp))
            return false;
        m_control_urls.push_back(track_url);

        // PLAY
        if (!SendRtspCommand("PLAY", m_base_url, "", resp))
            return false;
    ...
    bool SendRtp(const uint8_t *data, size_t len)
    {
        if (m_sock == INVALID_SOCKET)
            return false;
        // RTSP interleaved frame: $ + channel(1) + length(2, BE) + data
        uint8_t header[4];
        header[0] = '$';
        header[1] = 0; // channel 0 for RTP
        header[2] = (uint8_t)((len >> 8) & 0xFF);
        header[3] = (uint8_t)(len & 0xFF);
        int r = send(m_sock, (const char *)header, 4, 0);
        if (r != 4)
            return false;
        size_t sent = 0;
        while (sent < len)
        {
            r = send(m_sock, (const char *)(data + sent), (int)(len - sent), 0);
            if (r <= 0)
                return false;
            sent += r;
        }
...
private:
    ...
    std::vector<std::string> m_control_urls;

    bool SendRtspCommand(const std::string &method, const std::string &url,
                         const std::string &body, std::string &response)
    {
        ...
        req += "User-Agent: StreamingAgent/1.0\r\n";
        if (!body.empty())
        {
        ...
        if (send(m_sock, req.c_str(), (int)req.size(), 0) != (int)req.size())
            return false;
            ...
            if (r > 0)
            {
                ...
                if (response.find("\r\n\r\n") != std::string::npos)
                {
                    ...
                    if (cl_pos != std::string::npos)
                    {
                        ...
                        int cl = std::stoi(response.substr(cl_pos, cl_end - cl_pos));
                        size_t body_start = response.find("\r\n\r\n") + 4;
                        if (response.size() >= body_start + (size_t)cl)
                            break;
                    }
                    else
                        break;
        ...
        ULONG mode0 = 0;
        ioctlsocket(m_sock, FIONBIO, &mode0);
...
static void extract_sps_pps(const std::vector<uint8_t> &nal_buf,
                            std::vector<uint8_t> &sps, std::vector<uint8_t> &pps)
{
    sps.clear();
    pps.clear();
    size_t i = 0;
    while (i + 4 <= nal_buf.size())
    {
        // Find start code (0x00000001)
        if (nal_buf[i] == 0 && nal_buf[i + 1] == 0 &&
            (nal_buf[i + 2] == 1 || (nal_buf[i + 2] == 0 && nal_buf[i + 3] == 1)))
        {
            size_t nal_start = i;
            if (nal_buf[i + 2] == 0)
                i += 4;
            ...
            uint8_t nal_type = nal_buf[i] & 0x1F;
            // Find end of this NAL
            size_t end = nal_start + 1;
            while (end + 4 <= nal_buf.size())
            {
                if (nal_buf[end] == 0 && nal_buf[end + 1] == 0 &&
                    (nal_buf[end + 2] == 1 || (nal_buf[end + 2] == 0 && nal_buf[end + 3] == 1)))
                    break;
                end++;
            }

            if (nal_type == 7) // SPS
                sps.assign(nal_buf.begin() + nal_start, nal_buf.begin() + end);
            else if (nal_type == 8) // PPS
                pps.assign(nal_buf.begin() + nal_start, nal_buf.begin() + end);
...
int run_streaming_worker(const std::string &relay_host, int relay_port)
{
    log("run_streaming_worker: starting, relay=" + relay_host + ":" + std::to_string(relay_port));

    if (!WGCCapture::IsWGCSupported())
    {
        log("run_streaming_worker: WGC not supported on this system");
        return 1;
    }
    ...
    {
        log("run_streaming_worker: WGC initialization failed");
        return 1;
    }

    log("run_streaming_worker: WGC initialized, monitors=" + std::to_string(capture.GetMonitorCount()));

    H264RtpPacketizer packetizer;
    packetizer.SetSsrc(0xDEADBEEF);
    packetizer.SetPayloadType(RTP_PT_H264);
    packetizer.SetMaxPayloadSize(1400);

    int64_t frame_count = 0;
    std::string stream_name = "monitor0";

    while (!g_stopRequested)
    {
        // Capture frame from monitor 0
        std::vector<uint8_t> bgra;
        int w = 0, h = 0;
        if (!capture.CaptureFrame(0, bgra, w, h))
        {
            Sleep(33);
            continue;
        }
        ...
        // Initialize encoder on first frame (we know dimensions now)
        static X264Encoder encoder;
        static bool encoder_inited = false;
        if (!encoder_inited)
        {
            if (!encoder.Initialize(w, h, 15, 23))
            {
                log("run_streaming_worker: x264 encoder init failed");
                Sleep(1000);
                continue;
            }
            encoder_inited = true;
            log("run_streaming_worker: x264 encoder initialized, " +
                std::to_string(w) + "x" + std::to_string(h));
        }

        // Encode frame
        std::vector<uint8_t> nal;
        if (!encoder.EncodeFrame(bgra, nal))
            continue;

        if (nal.empty())
            continue;

        // Extract SPS/PPS; on first announce or reconnect, request a keyframe
        static bool announced = false;
        std::vector<uint8_t> sps, pps;
        extract_sps_pps(nal, sps, pps);
        ...
        if (!sps.empty() && !pps.empty() && !announced)
        {
            RtspPushClient client;
            if (!client.Connect(relay_host, relay_port))
            {
                log("run_streaming_worker: cannot connect to relay");
                encoder.RequestKeyframe();
                Sleep(1000);
                continue;
            }

            if (!client.Announce(stream_name, w, h, sps, pps))
            {
                log("run_streaming_worker: RTSP announce failed");
                client.Disconnect();
                encoder.RequestKeyframe();
                Sleep(1000);
                continue;
            ...
            log("run_streaming_worker: RTSP session established");

            // Send first frame's RTP packets
            uint32_t ts = (uint32_t)(frame_count * (90000 / 15));
            std::vector<std::vector<uint8_t>> packets;
            packetizer.Packetize(nal.data(), nal.size(), ts, packets);

            for (auto &pkt : packets)
            {
                if (!client.SendRtp(pkt.data(), pkt.size()))
                {
                    log("run_streaming_worker: send RTP failed on first frame");
                    break;
                }
            }
            ...
            // Keep sending until error or stop
            while (!g_stopRequested)
            {
                std::vector<uint8_t> next_bgra;
                int nw = 0, nh = 0;
                if (!capture.CaptureFrame(0, next_bgra, nw, nh))
                {
                    Sleep(33);
                    continue;
                }
                if (nw == 0 || nh == 0 || next_bgra.empty())
                {
                    Sleep(33);
                    continue;
                }

                std::vector<uint8_t> next_nal;
                if (!encoder.EncodeFrame(next_bgra, next_nal))
                    continue;
                if (next_nal.empty())
                    continue;

                ts = (uint32_t)(frame_count * (90000 / 15));
                packets.clear();
                packetizer.Packetize(next_nal.data(), next_nal.size(), ts, packets);
                ...
                if (!send_ok)
                {
                    log("run_streaming_worker: send failed, reconnecting");
                    client.Disconnect();
                    announced = false;
                    encoder.RequestKeyframe();
                    break;
    ...
    log("run_streaming_worker: stopped");
    return 0;
}
