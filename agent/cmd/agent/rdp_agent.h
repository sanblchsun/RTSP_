...
public:
    ...
    bool isStopRequested() const { return runtime.stop; }
...
public:
    ...
    std::string make_hello_json() const;
    ...
    // RTSP_ pipeline: single monitor capture → encode → RTSP push
    bool init_capture();
    void shutdown_capture();
    void capture_loop();
    void encode_and_push_loop();
    std::unique_ptr<CaptureBase> m_capture;
    std::unique_ptr<PerMonitorState> m_monitor;
